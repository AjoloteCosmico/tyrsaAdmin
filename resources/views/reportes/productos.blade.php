...
@section('content')     <div class="container-flex m-1 bg-gray-300 shadow-lg rounded-lg">
 ...
 <div style="text-transform: lowercase;"> info@tyrsa.com.mx <b> <a href="https://www.tyrsa.com.mx"> www.tyrsa.com.mx</a> </b></div>     <br>
            ...
            <h5 class="text-lg text-center text-bold">RESULTADOS MENSUALES DE VENTAS POR PRODUCTOS POR  {{$Monto}} </h5>
            <br>
            <div >
                <table>
                    <tr>
  ...
  <button id="btn-graficos" class="boton" onclick="mostrar('graficos', this)">Gráficos</button>
               ...
               <tr class="text-center">
                ...
                <tr>
                    ...
                    @for($i=1;$i<=12;$i++)
                      @if($Monto=='MONTO')                    
                        <td class="text-nowrap" > $ {{number_format($Items->where('products',$product->id)->where('date','>=',$Year.'-'.str_pad($i, 2, '0', STR_PAD_LEFT).'-01')->where('date','<=',$Year.'-'.str_pad($i, 2, '0', STR_PAD_LEFT).'-31')->sum('total'),2)}}  </td>
                        @else
                    ...
                    @endfor
                    @if($Monto=='MONTO')  
                      <td class="text-nowrap" > $ {{number_format($Items->where('products',$product->id)->sum('total'),2)}}</td>
                ...
                <tr>
                    ...
                    @for($i=1;$i<=12;$i++)
                      ...
                      @if($Monto=='MONTO')  
                        <th> $ {{number_format($Items->where('date','>=',$Year.'-'.str_pad($i, 2, '0', STR_PAD_LEFT).'-01')->where('date','<=',$Year.'-'.str_pad($i, 2, '0', STR_PAD_LEFT).'-31')->sum('total'),2)}}  </th>
                    ...
                    @endfor
                      @if($Monto=='MONTO')  
                      <th> $ {{number_format($Items->sum('total'),2)}} </th>
                      @else
                      <th>{{$Items->count()}} </th>
                      @endif
