  ...
  <button id="btn-graficos" class="boton" onclick="mostrar('graficos', this)">Gráficos</button>
               ...
               <tr class="text-center">
                ...
                <tr>
                    ...
                    @for($i=1;$i<=12;$i++)
                      @if($Monto=='MONTO')                    
                        <td class="text-nowrap" > $ {{number_format($InternalOrders->where('seller_id',$seller->id)->where('date','>=',$Year.'-'.str_pad($i, 2, '0', STR_PAD_LEFT).'-01')->where('date','<=',$Year.'-'.str_pad($i, 2, '0', STR_PAD_LEFT).'-31')->sum('exchange_total')/1.16,2)}}  </td>
                        @else
                    ...
                    @endfor
                    @if($Monto=='MONTO')  
                      <td class="text-nowrap" > $ {{number_format($InternalOrders->where('seller_id',$seller->id)->where('date','>=',$Year.'-01-01')->sum('exchange_total')/1.16,2)}}</td>
                      <td class="text-nowrap" >{{number_format(divide(100*$InternalOrders->where('seller_id',$seller->id)->sum('exchange_total'),$InternalOrders->sum('exchange_total')),2)}} %</td>
                    @else
                ...
                <tr>
                    ...
                    @for($i=1;$i<=12;$i++)
                      ...
                      @if($Monto=='MONTO')  
                        <th> $ {{number_format($InternalOrders->where('date','>=',$Year.'-'.str_pad($i, 2, '0', STR_PAD_LEFT).'-01')->where('date','<=',$Year.'-'.str_pad($i, 2, '0', STR_PAD_LEFT).'-31')->sum('exchange_total')/1.16,2)}}  </th>
                    ...
                    @endfor
                      @if($Monto=='MONTO')  
                      <th> $ {{number_format($InternalOrders->sum('exchange_total')/1.16,2)}} </th>
                      @else
