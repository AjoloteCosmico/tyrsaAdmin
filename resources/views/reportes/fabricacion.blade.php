  ...
  <button id="btn-graficos" class="boton" onclick="mostrar('graficos', this)">Gráficos</button>
               ...
               <tr class="text-center">
                ...
                <tr>
                    ...
                    @endfor
                    @if($Monto=='MONTO')  
                      <td class="text-nowrap" > $ {{number_format($InternalOrders->where('marca',$marca->id)->sum('total'),2)}}</td>
                      <td class="text-nowrap" >{{number_format(divide(100*$InternalOrders->where('marca',$marca->id)->sum('total'),$InternalOrders->sum('total')),1)}} %</td>
                    @else
                    
                    <td class="text-nowrap" >{{$InternalOrders->where('marca',$marca->id)->count()}}</td>
                      <td class="text-nowrap" > {{number_format(divide(100*$InternalOrders->where('marca',$marca->id)->count(),$InternalOrders->count(),1))}} %</td>
                    @endif
                </tr>
                @endforeach
                <tfoot>
