...
class NotasCreditoController extends Controller
{
    ...
    public function create(){
        //traer todas las facturas
                ...
                $Customers = DB::table('customers as c')
                    ->join('internal_orders as io', 'io.customer_id', '=', 'c.id')
                    ->select('c.id', 'c.clave', 'c.customer')
                    ->whereNotNull('io.customer_id')
                    ->groupBy('c.id', 'c.clave', 'c.customer')
                    ->orderBy('c.clave')
                    ->get();
    ...
    public function store(Request $request){
                ...
                if ($Facturado>=($TotalPedido-0.9)) {
                    return redirect()->back()->with('error','facturado');
                }
                $Nota=new CreditNote();
