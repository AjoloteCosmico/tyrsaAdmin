...
class VisualizacionController extends Controller
{
   public function pedidos_no_autorizados(){
    ...
    $sna1dia=InternalOrder::where('reg_date',now()->modify('-1 day'))->get()->count();
    $sna2dia=InternalOrder::where('reg_date',now()->modify('-2 day'))->get()->count();
    $sna3dia=InternalOrder::where('reg_date',now()->modify('-3 day'))->get()->count();
    $snamasdias=InternalOrder::where('reg_date','<',now()->modify('-3 day'))->get()->count();
    $grafica_autorizados = LarapexChart::barChart()
        ...
        ->setSubtitle('por dias desde su emisión')
         ...
         ->setXAxis(['1 día', '2 dias', '3 dias', '4 o más']);
