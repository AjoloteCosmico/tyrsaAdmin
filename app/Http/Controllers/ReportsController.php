...
class ReportsController extends Controller
{
       public function generate($id,$report,$pdf)
       {
           ...
           if($pdf==0){
            ...
            $process = new Process(["soffice --convert-to 'pdf:calc_pdf_Export:{\"SinglePageSheets\":{\"type\":\"boolean\",\"value\":\"true\"}}' public/storage/report/".$report.$id.".xlsx --outdir public/storage/report"]);
            $process->run();
            if (!$process->isSuccessful()) {
                throw new ProcessFailedException($process);
            }
