...
<?php $__env->startSection('title', 'REPORTE DE OBJETIVOS'); ?>
...
<?php $__env->startSection('content'); ?>     <div class="container-flex m-1 bg-gray-300 shadow-lg rounded-lg">
 ...
 <div style="text-transform: lowercase;"> info@tyrsa.com.mx <b> <a href="https://www.tyrsa.com.mx"> www.tyrsa.com.mx</a> </b></div>     <br>
            ...
            <h5 class="text-lg text-center text-bold">OBJETIVOS Y RESULTADOS POR <?php echo e($Monto); ?> </h5>
            <br>
            <div >
                <table>
                    <tr>
  ...
  <button id="btn-graficos" class="boton" onclick="mostrar('graficos', this)">Gráficos</button>
               ...
               <tr class="text-center">
                ...
                <tr>
                    ...
                    <?php for($i=1;$i<=12;$i++): ?>
                      <?php if($Monto=='MONTO'): ?>                    
                        <td class="text-nowrap" > $ <?php echo e(number_format($InternalOrders->where('seller_id',$seller->id)->where('date','>=',$Year.'-'.str_pad($i, 2, '0', STR_PAD_LEFT).'-01')->where('date','<=',$Year.'-'.str_pad($i, 2, '0', STR_PAD_LEFT).'-31')->sum('total')/1.16,2)); ?>  </td>
                        <?php else: ?>
                        <td><?php echo e($InternalOrders->where('seller_id',$seller->id)->where('date','>=',$Year.'-'.str_pad($i, 2, '0', STR_PAD_LEFT).'-01')->where('date','<=',$Year.'-'.str_pad($i, 2, '0', STR_PAD_LEFT).'-31')->count()); ?>  </td>
                        
                        <?php endif; ?>
                    <?php endfor; ?>
                    <?php if($Monto=='MONTO'): ?>  
                      <td class="text-nowrap" > $ <?php echo e(number_format($InternalOrders->where('seller_id',$seller->id)->where('date','>=',$Year.'-01-01')->sum('total')/1.16,2)); ?></td>
                      <td class="text-nowrap" ><?php echo e(number_format(divide(100*$InternalOrders->where('seller_id',$seller->id)->sum('total'),$InternalOrders->sum('total')),1)); ?> %</td>
                    <?php else: ?>
                    
                    <td class="text-nowrap" ><?php echo e($InternalOrders->where('seller_id',$seller->id)->count()); ?></td>
                      <td class="text-nowrap" > <?php echo e(number_format(divide(100*$InternalOrders->where('seller_id',$seller->id)->count(),$InternalOrders->count()),1)); ?> %</td>
                    <?php endif; ?>
                </tr>
                ...
                <tr>
                    ...
                    <?php for($i=1;$i<=12;$i++): ?>
                      ...
                      <?php if($Monto=='MONTO'): ?>  
                        <th> $ <?php echo e(number_format($InternalOrders->where('date','>=',$Year.'-'.str_pad($i, 2, '0', STR_PAD_LEFT).'-01')->where('date','<=',$Year.'-'.str_pad($i, 2, '0', STR_PAD_LEFT).'-31')->sum('total')/1.16,2)); ?>  </th>
                    ...
                    <?php endfor; ?>
                      <?php if($Monto=='MONTO'): ?>  
                      <th> $ <?php echo e(number_format($InternalOrders->sum('total')/1.16,2)); ?> </th>
                      <?php else: ?>
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('adminlte::page', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\admintyrsa2\tyrsaAdmin\resources\views/reportes/objetivos.blade.php ENDPATH**/ ?>