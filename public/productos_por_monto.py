...
creditos=pd.read_sql("""select * 
    ...
    inner join coins on internal_orders.coin_id = coins.id) """,cnx)
productos=pd.read_sql("""select items.*,internal_orders.date from (items inner join report_products on report_products.id =items.products) inner join internal_orders on items.internal_order_id=internal_orders.id""",cnx)
...
writer = pd.ExcelWriter('storage/report/productos_por_monto1.xlsx', engine='xlsxwriter')
...
#Columna para filtrar por fechas
productos['date']=pd.to_datetime(productos['date'])
productos=productos.loc[productos['date']>year+'-01-01']
df[0:1].to_excel(writer, sheet_name='Sheet1', startrow=7,startcol=6, header=False, index=False)
worksheet = writer.sheets['Sheet1']
#Encabezado del documento--------------
worksheet.merge_range('B2:F2', 'CUENTAS POR COBRAR REPORTE 6/8', negro_b)
worksheet.merge_range('B3:F3', 'PRODUCTOS POR MONTO', negro_s)
...
for i in range(len(productos)):
    product_id=productos['id'].values[i]
    pxv=pedidos.loc[pedidos['seller_id']==product_id] 
    worksheet.write('B'+str(7+i), productos['name'].values[i], blue_content)
    for mes in range(12):
        li=year+'-'+str(mes+1)+'-01'
        ls=year+'-'+str(mes+2)+'-01'
        if(mes+1==12):
            ls=str(int(year)+1)+'-01-01'
        worksheet.write(6+i,mes+2, pxv.loc[(pxv['date']<ls)&(pxv['date']>=li),'import'].sum()/1.16, blue_content)
    worksheet.write('O'+str(7+i), pxv['import'].sum()/1.16, blue_content)
    worksheet.write('P'+str(7+i), str(round((pxv['import'].sum()*100)/pedidos['import'].sum(),2))+'%', blue_content)
     
worksheet.write('B'+str(len(productos)+8), 'Total Mensual', blue_header_format)
for i in ['C','D','E','F','G','H','I','J','K','L','M','N','O']:

    worksheet.write_formula(i+str(len(productos)+8),  '{=SUM('+i+'7:'+i+str(len(productos)+6)+')}',blue_content_bold)
worksheet.write('P'+str(len(productos)+8),  '100%',blue_content_bold)




worksheet.write('B'+str(len(productos)+10), 'Objetivo Anual (MONTO)', blue_header_format)
worksheet.merge_range('C'+str(len(productos)+10)+':D'+str(len(productos)+10), objetivo/1.16, blue_content_bold)

worksheet.write('B'+str(len(productos)+11), 'Dias transcurridos', blue_header_format)
worksheet.merge_range('C'+str(len(productos)+11)+':D'+str(len(productos)+11), str(dias_transcurridos.days), blue_content_bold)

worksheet.write('B'+str(len(productos)+12), 'Objetivo a la fecha', blue_header_format)
worksheet.merge_range('C'+str(len(productos)+12)+':D'+str(len(productos)+12), pedidos['total'].sum(), blue_content_bold)

worksheet.write('B'+str(len(productos)+13), 'Porcentaje completado', blue_header_format)
worksheet.merge_range('C'+str(len(productos)+13)+':D'+str(len(productos)+13), str(round(pedidos['total'].sum()*100/objetivo,2))+'%', blue_content_bold)
#Grafica
...
chart.add_series({'values': '=Sheet1!$O$7:$O$'+str(7+ len(productos)),
                  'categories': '=Sheet1!$B$7:$B$'+str(7+ len(productos)),
                  'percentage': True,
                    'leader_lines': True,
                    'position': 'best_fit',
                    'data_labels': {
