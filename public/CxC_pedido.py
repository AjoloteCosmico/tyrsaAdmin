...
#facturado, logica 1-B
   if(pedidos['coin_id'].values[i]==1):
        if(pedidos['total'].values[i]- cobros.loc[cobros['order_id']==pedidos['id'].values[i],'amount'].sum()-1<=0):
            ...
            fact=(facturas.loc[facturas['order_id']==pedidos['id'].values[i],'amount'].sum()/1.16-creditos.loc[creditos['order_id']==pedidos['id'].values[i],'amount'].sum()/1.16)-cobros.loc[cobros['order_id']==pedidos['id'].values[i],'amount'].sum()/1.16
...
#         worksheet.write('S'+row_index, max(0,(pedidos['total'].values[i]/1.16- facturas.loc[facturas['order_id']==pedidos['id'].values[i],'amount'].sum()/1.16+creditos.loc[creditos['order_id']==pedidos['id'].values[i],'amount'].sum()/1.16)), blue_content_dll)
   ...
   #por facturar modified pa cuadrar
   if(pedidos['coin_id'].values[i]==1):
        worksheet.write('R'+row_index,total/1.16 - cobros.loc[cobros['order_id']==pedidos['id'].values[i],'amount'].sum()/1.16-fact, blue_content)
        worksheet.write('S'+row_index, 0, blue_content_dll)
   else:
        worksheet.write('R'+row_index, 0, blue_content)
