...
df=pedidos[['date']]
orders=pedidos.loc[pedidos['status']=='CAPTURADO']
tc=pd.read_sql('select * from coins where id=13 ',cnx)['exchange_sell'].values[0]
writer = pd.ExcelWriter('storage/report/pedidos_sn_aut'+str(id)+'.xlsx', engine='xlsxwriter')
...
worksheet.merge_range('B2:F2', 'PEDIDOS INTERNOS SIN AUTORIZAR ', negro_b)
...
worksheet.merge_range('C8:C10', 'PDA \n NHO \n 2022', blue_header_format)
worksheet.merge_range('D8:D10', 'FECHA DE EMISION  \n DD-MM-AA', blue_header_format)
worksheet.merge_range('E8:E10', 'PEDIDO INTERNO NO.', blue_header_format)

worksheet.merge_range('F8:G8', 'CLIENTE', blue_header_format)
worksheet.merge_range('F9:F10', 'NUMERO', blue_header_format)
worksheet.merge_range('G9:G10', 'NOMBRE CORTO', blue_header_format)
worksheet.merge_range('H8:H10', 'MONEDA', blue_header_format)

worksheet.merge_range('I8:I10', 'IMPORTE TOTAL SIN IVA', blue_header_format)
worksheet.merge_range('J8:J10', 'STATUS', blue_header_format)
worksheet.merge_range('K8:K10', 'DIAS SIN AUTORIZAR', blue_header_format)
acumulado=0

#Llenar la tabla
for i in range(0,len(orders)):
     
     worksheet.write(10+i, 2, i+1,blue_content)
     worksheet.write(10+i, 3, str(orders["reg_date"].values[i]),blue_content)
     worksheet.write(10+i, 4, str(orders["invoice"].values[i]),blue_content)
     worksheet.write(10+i, 5, str(orders["clave"].values[i]),blue_content)
     worksheet.write(10+i, 6, str(orders["alias"].values[i]),blue_content)
     worksheet.write(10+i, 7, str(orders["coin"].values[i]),blue_content)
     worksheet.write(10+i, 8, orders["total"].values[i]/1.16,blue_content)
     worksheet.write(10+i, 9, orders["status"].values[i],blue_content)
     worksheet.write(10+i, 10, str((date - orders["date"].values[i]).days),blue_content)
     

worksheet.merge_range('C'+str(16+len(orders))+':E'+str(17+len(orders)), 'TOTAL PEDIDOS SIN AUTORIZAR', blue_header_format)
worksheet.merge_range('F'+str(16+len(orders))+':G'+str(17+len(orders)), str(len(orders)), blue_content_bold)
